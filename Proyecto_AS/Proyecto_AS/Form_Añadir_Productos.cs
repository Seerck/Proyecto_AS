...
namespace Proyecto_AS
{
    public partial class Form_Añadir_Productos : Form
    {
        ...
        static string inicio_sesion = "Server=LAPTOP-OBQGVQ1D ;Database=BD_AS ;User id=sa ;Password=2421;";
        ...
        {
            if (nombrecmd.Text != "" && tipocmd.Text != "" && cantidadcmd.Text != "" && ubicacioncmd.Text != "" &&
    fechaingresocmd.Text != "" && fechasalidacmd.Text != "" && estadocmd.Text != "")
            {
                string cmd = "INSERT INTO PRODUCTO (Nombre, Tipo, Cantidad, Ubicacion, FechaIngreso, FechaSalidad, Estado) " +
                     "VALUES ('" + nombrecmd.Text + "','" + tipocmd.Text + "','" + cantidadcmd.Text + "','" + ubicacioncmd.Text + "','" + fechaingresocmd.Text + "','" + fechasalidacmd.Text + "','" + estadocmd.Text + "')";
                SqlCommand lol = new SqlCommand(cmd, conectar);
                SqlDataAdapter data = new SqlDataAdapter(lol);
                DataTable red = new DataTable();
                data.Fill(red);
