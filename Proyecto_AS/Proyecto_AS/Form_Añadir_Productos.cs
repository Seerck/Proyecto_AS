...
namespace Proyecto_AS
{
    public partial class Form_Añadir_Productos : Form
    {
        ...
        {
            string cmb = "select * from PRODUCTO WHERE Nombre = '" + nombrecmd.Text + "' AND  Cantidad = '"+ txt_cantidad.Text + "' AND Precio ='" + txt_precio.Text + "' AND Ubicacion ='"+ ubicacioncmd.Text + "' AND Tipo = '"+ cmdtipo.Text +"'";
            SqlCommand lol = new SqlCommand(cmb, conectar);
        ...
        {
            consultar();
            dataGridView1.ClearSelection(); // Esto desactiva cualquier selección
        }
